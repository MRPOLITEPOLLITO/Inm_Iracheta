...
<?php
  ...
  $lat = substr($info['Map'], 1, strpos($info['Map'],",")-1);
  $alt = substr($info['Map'], strpos($info['Map'],",")+2, -1);
  $total = $lat + $alt;
  mysqli_close($con);
?>
...
<script>
  let latd = <?php echo $lat ?>;
  let altd = <?php echo $alt ?>;
  let coords = {lat: latd, lng: altd};
  const mapDiv = document.getElementById("mapita");
  let map;
  let marker;
  let image = "images/mapa.png";
  function initMap(){
